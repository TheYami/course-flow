...
export default function PaymentPage() {
  ...
  const handlePromotionSubmit = async (e) => {
    ...
    try {
      ...
      if (error) {
        alert("promotion code invalid");
        setLoading(false);
      } else {
        setPromotionData(data);

        if (data.course_id !== courseData.course_id) {
          alert("This promotion code cannot be used with this course");
          setLoading(false);
          return;
        } else {
        ...
        if (data.discount_type === "Fixed amount") {
          let total = courseData.price - data.discount;
          if (total <= 0) {
            setTotal(0);
          } else {
            setTotal(total);
          ...
          let discounts = courseData.price * (data.discount / 100);
          let total = Math.floor(courseData.price - discounts);
          if (total <= 0) {
            setTotal(0);
          } else {
            setTotal(total);
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      ...
      console.log('session uid is :',session.user);
  ...
  return (
    ...
    <div className="px-4 lg:px-0">
      <div className="pt-4 lg:pt-8 pb-10 lg:pb-60 xl:px-48 flex flex-col sm:px-4">
        <Link href={`/course/${slug}`} className="flex gap-2 no-underline text-[#2F5FAC] font-bold hover:text-blue-500 w-20 px-2 py-1 items-center">
        ...
        <h2 className="text-2xl lg:text-4xl font-medium pt-4">Enter payment info to start <br /> your subscription</h2>

        <p className="text-[#646D89] pt-10 lg:pt-12">Select payment method</p>

        {isClient && (
          <form onSubmit={handlePaymentSubmit} className="pt-4 flex flex-col lg:flex-row gap-4">
            <div className="flex flex-col gap-3">
              <div className={`flex flex-col gap-2 pb-4 lg:pb-10 rounded-lg ${methodPayment === "credit-card" ? "bg-[#F1F2F6]" : "bg-[#fff]"}`}>
                ...
                <div className="flex flex-col gap-6 px-6 md:px-16">
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex flex-col md:flex-row gap-2">
                      <div className={`py-3 px-4 bg-white rounded-lg w-[285px] md:w-[453px] border-[1px] ${cardNumberError ? 'border-[#9B2FAC]':'border-[#D6D9E4]'}`}>
                        <CardNumberElement
                          id="card-number"
                          options={{
                            style: {
                    ...
                    {nameOnCardError && <p className={` block lg:hidden text-sm text-[#9B2FAC] px-3 m-0 `}>Please fill out this field</p>}

                      <div className="flex gap-2">
                        <Image src={visa} alt="VISA Image" width={48} height={48} />
                        <Image src={mastercard} alt="Mastercard Image" width={48} height={48} />
                      </div>
                    </div>
                    {nameOnCardError && <p className={`hidden lg:block text-sm text-[#9B2FAC] px-3 m-0 `}>Please fill out this field</p>}
                  </div>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      className={`py-3 px-4 outline-none border-[1px] ${nameOnCardError ? 'border-[#9B2FAC]':'border-[#D6D9E4]'} rounded-lg w-[285px] md:w-[453px]`}
                    />
                    {nameOnCardError && <p className={`text-sm text-[#9B2FAC] px-3 m-0 `}>Please fill out this field</p>}
                  </div>
                  ...
                  <div className="flex gap-2 lg:gap-4">
                    <div className="flex flex-col gap-1">
                      ...
                      <div className={`px-4 py-3 bg-white rounded-lg w-[139.5px] md:w-[218.5px] border-[1px]  ${cardExpiryError ? 'border-[#9B2FAC]':'border-[#D6D9E4]'}`}>
                        <CardExpiryElement
                          id="card-expiry"
                          options={{
                            style: {
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div className={`px-4 py-3 bg-white border-[1px] ${cardCvcError ? 'border-[#9B2FAC]':'border-[#D6D9E4]'} rounded-lg w-[139.5px] md:w-[218.5px]`}>
                        <CardCvcElement
                          id="card-cvc"
                          options={{
                            style: {
                      ...
                      {cardCvcError && <p className={`text-sm text-[#9B2FAC] px-3 m-0 `}>Please fill out this field</p>}
                    </div>
                  </div>
                </div>
              </div>

              <div className={`'rounded-lg ${methodPayment === "qrcode" ? "bg-[#F1F2F6]" : "bg-[#fff]"}`}>
            ...
            <div className="py-8 px-2 lg:px-6 flex flex-col rounded-lg shadow-sm gap-6">
              ...
              <div className="flex flex-col">
                <h4 className="text-[#646D89] text-base font-normal leading-6">Subscription</h4>
              ...
              <div className="flex flex-col gap-2.5">
                <form className="flex gap-2 lg:gap-4 pb-1 md:justify-between">
                  <input
                    ...
                    className="py-3 px-4 w-[191px] md:w-[500px] lg:w-[205px] outline-none rounded-lg border border-[#D6D9E4]"
                  />
                  <button
                    type="button"
                    onClick={handlePromotionSubmit}
                    className={`py-[18px] px-8 rounded-xl ${promotionCode === "" ? "bg-[#D6D9E4] text-[#9AA1B9]" : "bg-[#2F5FAC] text-[#FFFFFF]"}`}
                  >
                    Apply
                  </button>
                </form>
                ...
                <div className="flex justify-between">
                  ...
                  <h3 className="text-base font-normal text-[#646D89]">{formatSubtotal(courseData.price)}</h3>
                </div>
                ...
                {discount !== 0 && (
                  <div className="flex justify-between">
                    ...
                    <h3 className="text-base font-normal text-[#9B2FAC]">
                      -{`${discount} ${promotionData.discount_type === "Percent" ? "%" : ""}`}
                    </h3>
                  </div>
                )}
                ...
                <div className="flex justify-between">
                  ...
                  <h3 className="text-base font-normal text-[#646D89] text-right">
                    {methodPayment === "credit-card" ? (
                      <>
                        Credit card / Debit <span className="block">card</span>
                      </>
                    ) : (
                      "QR Payment"
                    )}
